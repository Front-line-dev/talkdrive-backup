...
- 서버에서 삭제하지 않으므로 `drawerId` 기반 offset 페이징으로 다음 페이지를 넘김
- **위치 자동 저장/재개**: 타입별로 마지막 `drawerId`를 파일(`download_cursor_{TYPE}.txt`)에 저장하여 다음 실행 시 이어서 진행
...
├── download_cursor_MEDIA.txt   # 타입별 offset 위치 (download_only.py용)
...
- **totalCount와 실제 다운로드 수 차이**: API의 `totalCount`는 퇴장한 채팅방 등 접근 불가 항목을 포함합니다. 실제 다운로드 가능한 수는 이보다 적을 수 있습니다. 최종 결과에서 "조회 가능"과 "접근 불가" 수를 확인할 수 있습니다.
